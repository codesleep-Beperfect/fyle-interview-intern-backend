from marshmallow import Schema, EXCLUDE, fields, post_load, validates_schema, ValidationError
...
class AssignmentSchema(SQLAlchemyAutoSchema):
    ...
    def validate_content(self, data, **kwargs):
        if data.get('content') is None:
            raise ValidationError('Content cannot be null', field_name='content')
...
class TeacherListSchema(SQLAlchemyAutoSchema):
    class Meta:
        model = Teacher
        unknown = EXCLUDE
    id = auto_field(required=False, allow_none=True)
